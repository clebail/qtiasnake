...
public:
    AStar(Game *game);
    QList<Game::Direction> getChemin(const QPoint& start, const QPoint& goal);
private:
    ...
    QMap<QString, QPair<QPoint, Game::Direction> > cameFrom;

    double heuristique(const QPoint& start, const QPoint& goal) const;
    QString key(int x, int y) const;
    QList<Game::Direction> reconstuire(const QPoint& goal) const;
};

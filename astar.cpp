...
typedef struct _Direction {
    ...
    _Direction(int dx, int dy, Game::Direction direction) {
        this->dx = dx;
        this->dy = dy;
...
static Direction directions[NB_DIRECTION] = { Direction(0, -1, Game::edHaut), Direction(1, 0, Game::edDroite), Direction(0, 1, Game::edBas), Direction(-1, 0, Game::edGauche) };

AStar::AStar(Game *game) {
    this->game = game;
}

QList<Game::Direction> AStar::getChemin(const QPoint& start, const QPoint& goal) {
    int sx = start.x();
    int sy = start.y();
    int gx = goal.x();
    int gy = goal.y();
    ...
    while(openList.size() > 0) {
        ...
        for(int i=0;i<NB_DIRECTION;i++) {
            ...
            if (!scores.contains(vKey) || tentative < scores.value(vKey)) {
                ...
                cameFrom.insert(vKey, QPair<QPoint, Game::Direction>(QPoint(u.x, u.y), dir.direction));
...
QList<Game::Direction> AStar::reconstuire(const QPoint& goal) const {
    QList<Game::Direction> chemin;
